    ...
    if await is_active_chat(message.chat.id):
        ...
        if not await join_video_stream(message.chat.id, file, 720):
            return await mystic.edit(
                "خطاء في دخول الدردشت الصوتيه الرجاء التاكد ان الدردشه الصوتيه مفعله."
            )
        ...
        await mystic.delete()
        cap = f"🎥<b>__تشغيل:__ </b>[احصل على معلومات إضافية]({link})\n👤**__طلب من:__** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
...
async def start_video_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        if not await join_video_stream(
            ...
            return await CallbackQuery.message.reply_text(
                f"خطاء في دخول المحادثه الصوتيه."
            )
